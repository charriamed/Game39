...
public class CommandAdmin extends AdminUser {
    ...
    public void command(String command, String[] infos, String msg) {
        ...
        else if(command.equalsIgnoreCase("SERV"))
	    {
		  
		  String cmdAdm=msg.substring(5);
			  ...
			  if(infos.length>1)
		        {
		           if(cmdAdm==null)
		          {
		            this.sendMessage("Packet Commande Vide");
		            return;
		          }
		           try {
					this.getPlayer().getGameClient().parsePacket(cmdAdm);
